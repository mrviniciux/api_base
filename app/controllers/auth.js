...
export const signIn = async (req, res) => {
  const arrUserSelect = await db[dbName].query('SELECT * FROM user where name = ? and pwd = ?', 
  ...
  if(arrUserSelect.length > 0) {
    ...
    res.status(200).json({ data: {auth: true, token: token, username: arrUserSelect[0].username} });
  } else {
